...
renege hand currentCards previousState 
  ...
  | haveLeadingSuit && previousHaveQueen = getHighestWinCard probabilitiesHand leadSuit
  -- if I have Queen
  | handHaveQueen = (Card Spade Queen)
  -- if I have hearts, throw the highest, to reduce risk
  | handHaveHearts = getHighestWinCard probabilitiesHand Heart
  ...
  where 
    ...
    balanceCards = remainingCards (currentCards ++ previousCards ++ hand) 
    trickSize = getTrickSize currentCards
    probabilitiesHand = probabilitiesSearch filteredHand currentCards balanceCards trickSize

getMaxChance :: [(Card, Int)] -> Int
getMaxChance pairs = foldl (\a (_, b) -> if b > a && b >= 0 then b else a) (snd $ head pairs) pairs

-- [(SA,0),(S5,15),(S10,5),(HK,15),(HA,15)]
-- [(Card Spade Ace, 0), (Card Spade Five, 15), (Card Spade Ten, 5), (Card Heart King, 15), (Card Heart Ace, 15)] 
getHighestWinCard :: [(Card, Int)] -> Suit -> Card
getHighestWinCard handPairs suit = maximum maxCards
  where 
    result = filter (\(card, _) -> getSuit card == suit) handPairs
    maxChance = getMaxChance handPairs
    filteredMax = filter (\(_, a) -> a == maxChance) result
    maxCards = map fst filteredMax
...
getTrickSize :: [Card] -> Int
getTrickSize current = 4 - length current
